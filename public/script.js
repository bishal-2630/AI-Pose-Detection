...
async function goToDetectionPage(exerciseType) {
    ...
    try {
        ...
        // Reset initialization flag to force fresh start
        isInitialized = false;
...
async function loadMediaPipeDependencies() {
    return new Promise((resolve, reject) => {
        ...
        poseScript.type = 'text/javascript';
        ...
        poseScript.onload = () => {
            ...
            cameraScript.type = 'text/javascript';
            ...
            cameraScript.onload = () => {
                ...
                const checkInterval = setInterval(() => {
                    if (window.Pose && window.Camera) {
                        ...
                        console.log('MediaPipe global objects ready');
                        resolve();
                    }
                }, 50);
...
async function initializeAndStartCamera() {
    ...
    try {
        ...
        if (!window.Pose || !window.Camera) {
            await loadMediaPipeDependencies();
        }
        
        // Initialize MediaPipe if not already done
        if (!pose) {
            const poseInitialized = await initializeMediaPipe();
            if (!poseInitialized) {
    ...
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            if (videoElement.videoWidth > 0) {
                ...
                onError(new Error('Video timeout'));
            }
        }, 3000);
    });
}
...
function onPoseResults(results) {
    ...
    if (results.poseLandmarks && results.poseLandmarks.length > 0) {
        ...
        // Only show status when no arms are detected or when initializing
        ...
        if (leftArmVisible || rightArmVisible) {
            // Arms detected, don't show any message
            updateStatus('');
        } else {
            updateStatus('Move arms into frame');
        }
...
function calculateConfidence(results) {
    if (!results.poseWorldLandmarks || results.poseWorldLandmarks.length === 0) return 0;
...
function countExercise(angle, side) {
    ...
    if (selectedExercise === 'bicep_curl') {
        ...
        if (!areElbowsBelowShoulders(landmarks)) return;
        ...
        if (!areHandsAboveShoulders(landmarks)) return;
        ...
        // PUSH-UP SPECIFIC LOGIC: Count when arm goes from extended (>150) to bent (<90)
        // Reset repInProgress when back to extended position
        if (side === 'left' && angle > 150) {
            leftStageState = "up";
            updateLeftStage(config.upLabel, config.upColor);
            updateLeftStageProgress(1);
            repInProgress = false;
        ...
        // Count when bending
        if (side === 'left' && angle < 90 && leftStageState === "up") {
            leftStageState = "down";
            leftArmCounter++;
            updateLeftCounter();
            updateTotalReps();
        ...
        if (side === 'right' && angle < 90 && rightStageState === "up") {
            rightStageState = "down";
            rightArmCounter++;
            updateRightCounter();
            updateTotalReps();
...
function areElbowsBelowShoulders(landmarks) {
    ...
    // Check if elbows are below shoulders
...
function updateTitleAngles() {
    if (leftAngleTitle) leftAngleTitle.textContent = `${Math.round(leftArmAngle)}°`;
    if (rightAngleTitle) rightAngleTitle.textContent = `${Math.round(rightArmAngle)}°`;
}

function updateLeftArmUI(angle, visible = true) {
    if (!leftAngleValue || !leftAngleFill) return;
...
function updateRightArmUI(angle, visible = true) {
    if (!rightAngleValue || !rightAngleFill) return;
...
function updateLandmarkStats(count) {
    if (detectedPoints) detectedPoints.textContent = count;
    if (landmarkTitle) landmarkTitle.textContent = `Points: ${count}/33`;
    
    if (landmarksProgress) {
...
function updateStatus(message) {
    if (poseStatus) {
        if (message && message.trim() !== '') {
            ...
            poseStatus.textContent = statusMessage;
    ...
    // Keep detection status for the stats panel
    if (detectionStatus && message && message.trim() !== '') {
...
function updateProcessingMode() {
    if (processingMode) {
        processingMode.textContent = 'Browser';
    }
}
...
function resetAll() {
    ...
    if (totalRepsDisplay) totalRepsDisplay.textContent = '0';
    
    if (leftAngleValue) leftAngleValue.textContent = '0°';
    if (rightAngleValue) rightAngleValue.textContent = '0°';
...
window.addEventListener('beforeunload', () => {
    // Stop camera but don't close MediaPipe to avoid errors
    if (camera) {
        camera.stop();
    }
    if (mediaStream) {
